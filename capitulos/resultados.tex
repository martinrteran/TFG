...
%Como último caso de simulación se simula la transmisión de calor en CFD por conducción para una nTPV de nano-espaciador de $Si$ y un emisor de $Si$, $SS$ y $SiC$ con resistencias de contacto empíricas de $4\cdot 10^{-6} \ m^2 K/W$, por si durante el proceso de deposición del nano-espaciador no se llegara a conseguir depositar $SiO_2$ sino un material con características semejantes a las del $Si$. Se decide incluir este caso de simulación en vista de los resultados obtenidos en un TFM que se desarrolló en paralelo a este trabajo. En dicho TFM se depositaron capas de SiO2 por sputtering. Sin embrago, al evaluar las propiedades de las capas depositadas, estás se parecían más a las esperadas en una capa de Si que en una capa de SiO2.
...
%\begin{table}[H]
	%\centering
	%\caption{Tabla de recopilación de los resultados de las simulaciones de transferencia de calor por conducción para las nTPV con nano-espaciador de Si y emisores de $Si$, $SS$ y $SiC$ con $R_c$ empírica y para una nTPV de emisor de $SiC$ sin $R_c$ respecto a cada altura del nano-espaciador.}
...
%Por último, se calculan las densidades de nano-espaciadores frente a las alturas de los nano-espaciadores para la nTPV de emisor de $SiC$ y nano-espaciadores de $Si$ (figuras \ref{fig:prc_SiCXX} \subref{fig:rel_SiC_SinRc} y \subref{fig:rel_SiC}), y se compara con las densidades de la nTPV de emisor de $SiC$ pero nano-espaciadores de $SiO_2$ (figuras \ref{fig:relation_SiCSiO2Ge} \subref{fig:rel_SiCSiO2Ge} y \subref{fig:rel_SiCSiO2Ge_Rc}). Se observa que con $R_c$ existe una disminución de las densidades para cada relación de potencias y todas las alturas de los nano-espaciadores, siendo la más notable a los 1000 nm de altura y relación de potencias de un orden de magnitud, pasando la densidad de ser $12 \ n^{\circ}esp/cm^2$ (figura \ref{fig:rel_SiC}) a ser $10 \ n^{\circ}esp/cm^2$ (figura \ref{fig:rel_SiCSiO2Ge_Rc}). Esta poca variación para dos materiales distintos de nano-espaciadores se dá porque para ambos casos la nTPV tiene una $R_c$ que es la resistencia térmica más significativa del sistema.
%\begin{figure} [h]%
	%\centering
	%\begin{subfigure}[b]{0.48\textwidth}%
			%\includegraphics[width=\columnwidth]{rel_SiC_SinRc}%
			%\caption{}%
			%\label{fig:rel_SiC_SinRc}%
	%\end{subfigure}
	%\hfill
	%\begin{subfigure}[b]{0.48\textwidth}%
			...
			%\label{fig:rel_SiC}%
	%\end{subfigure}
	%\caption[Relación entre la potencia radiada para energías mayores e igual a 0.7 eV y la potencia transmitida por conducción con $R_c$ para distintas densidades de nano-espaciadores (eje y) y distintas alturas de los mismos (eje x), para nTPV de 1 $cm^2$ con nano-espaciadores de $Si$ con (\subref{fig:rel_SiC}) y sin $R_c$ (\subref{fig:rel_SiC_SinRc}). 
	...
	%\label{fig:prc_SiCXX}%
%\end{figure}
%En la figura \ref{fig:rel_SiC_SinRc} se observa como la tendencia de la curva es aumentar la densidad de nano-espaciadores al disminuir la altura de este, esto se produce porque la $h_c$ del $Si$ disminuye con el aumento de la temperatura, produciendo una disminución de la potencia transmitida por conducción al calentarse. En la tabla \ref{tab:nanoEspaciadorDeSi} se puede observar como la potencia transmitida por conducción para el caso de emisor de $SiC$ sin $R_c$ ($ $) es mayor que para el caso con nano-espaciador de $SiO_2$ (tabla \ref{tab:nanoEspaciadorDeSi}), pero teniendo una variación pico a pico menor.